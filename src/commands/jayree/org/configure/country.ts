...
export default class UpdateCountry extends SfCommand<void> {
  ...
  public async run(): Promise<void> {
    ...
    const setHTMLInputElementValue = async (newvalue: string, element: string): Promise<void> => {
      element = element.replace(/:/g, '\\:');
      const elementDisabled = await page.evaluate<boolean, string>((s) => {
    ...
    try {
      ...
      const table = await page.evaluate<string | undefined>(() => {
      ...
      const list = tabletojson.convert(table as string)[0] as Array<{
        Action: string;
        Active: string;
        Visible: string;
        Country: string;
        'Country/Territory Code': string;
        'Available States/Provinces': string;
      }>;
